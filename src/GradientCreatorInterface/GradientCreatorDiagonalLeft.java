...
public class GradientCreatorDiagonalLeft extends GradientCreator {

        public GradientCreatorDiagonalLeft() {
                super("Diagonal-Gauche", true);
        }
        ...
        public float[][] generateBlendTable(int x_dim, int y_dim, Color color1, Color color2, double colorIntensityParam, double param1, double param2) {
                ...
                for (int y = 0; y < y_dim; y++) {
                        for (int x = 0; x < x_dim; x++) {
                                blendTable[x][y] = (float) Math.pow((float)(x+y)/(x_dim+y_dim-2),colorIntensityParam);
